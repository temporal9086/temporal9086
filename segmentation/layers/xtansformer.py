            ...
            for idx in range(4):
                ...
                if idx == 0:
                    ...
                    for i in range(sterm):
                        temp_mean += node_mean[i::sterm]
                        temp_std += node_std[i::sterm].exp() + node_mean[i::sterm]**2
                    temp_mean = temp_mean/sterm
                    temp_std = torch.log(temp_std/sterm - temp_mean**2)
                    for i in range(sterm):
                        tnodes[:,i::sterm] = (temp_mean.expand(B,-1,-1) + torch.exp(temp_std*.5).expand(B, -1, -1)*torch.randn_like(temp_std, device=node_std.device, dtype=node_std.dtype))
                    ...
                    q = layer(q, kv)
                    # q_tmp = (q[0::2] + q[1::2])/2
        kl = self.kl_divergence(temp_mean, temp_std)
        reg = self.reg_logsigma(temp_std)
        term = 2**(idx)
        q_tmp = 0
        for i in range(term):
            q_tmp += q[:,i::term]
