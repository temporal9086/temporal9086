...
import lorentztreenet as lorentz
from layers.xtansformer import XTransformerEncoder
...
class VHMDEIT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=80, embed_dim=768, depth=12,
        ...
        if self.training:
            self.mix_l = XTransformerEncoder(visual_dim=self.num_features,transformer_layers=4)
            self.last_l = lorentz.HyboNet(
            c=1,
            bdim = self.num_features,
            dim=128,
            depth=4,
            temp=None)
    ...
    def forward_features(self, x):
        ...
        x = self.norm(x)
        if self.training:
            hyp_x = self.last_l(x, skip_head=False)
            x_hyp = self.mix_l(x, hyp_x[0])
            x_mix = x_hyp[:,-1]
            # col_loss = torch.abs(torch.mean(la.norm(x[:,0], ord=2, dim=1)/10 - la.norm(x_mix.squeeze(1), ord=2, dim=1)))
            x_out = self.head(1*x[:,1:] + 0.5*x_mix)
            # x = self.head(x)
            # print("recon : {}".format(recon))
            # return x_out, x[:,1:], col_loss/100, hyp_x
        else:
            hyp_x = self.last_l(x, skip_head=True)
            x_hyp = self.mix_l(x, hyp_x[0])
            x_mix = x_hyp[:,-1,1:]
            x_out = self.head(1*x[:,1:] + 0.5*x_mix)
            # return x_out, x[:,1:], hyp_x[2], x_hyp[:,:,0].unsqueeze(2)
        xp = x_out.permute(0, 2, 1).reshape(B, -1, Hp, Wp)
        
        ops = [self.fpn1, self.fpn2, self.fpn3, self.fpn4]
        for i in range(len(ops)):
            features.append(ops[i](xp))

        return tuple(features), hyp_x[1]
